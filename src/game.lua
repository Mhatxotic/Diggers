...
local abs<const>, ceil<const>, error<const>, floor<const>, format<const>,
  ...
  remove<const>, tostring<const>, sin<const> =
...
local function InitCreateObject()
  ...
  local function AIFindTarget(oObj)
    ...
    if oTarget then
      ...
      if oObj.AT >= 1800 or oTarget.F & iFPhaseTarget == 0 then
    ...
    if oObj.F & iFInWater == 0 then
      ...
      repeat
        ...
        if iId and aTileData[1 + iId] & iTFWater ~= 0 then
          ...
          -- Never fall down the gap when there is water
          break;
        end
        ...
        if IsCollide(oObj, iAdjX, 14) then
          ...
          -- Return if simulated jump failed
          if vResult == nil then return end;
          -- If not intelligent enough to try to jump?
          if vResult == false then
            -- Turn around if intelligent enough
            if random() >= oObj.IN then break end;
            -- Fall down
            return;
          end
          -- Jump success otherwise
          return true;
        end
        -- Increase Y position and fall damage. We go at 14 pixels each time as
        -- the bitmask sprite is 14 pixels high.
  ...
  local function AIFish(oObj)
    ...
    local iAdjY<const> =
      max(-2, min(2, floor(sin(iGameTicks * 0.1) * 2 + 0.5)));
    -- Return if we can't move in the requested direction
    if IsCollideY(oObj, iAdjY) then return end;
    -- Move in the specified direction if still in water
    if ObjectInWater(oObj, iAdjY) then AdjustPosY(oObj, iAdjY) end;
    -- If we can or should drop one pixel extra then do so
    if iAdjY > 0 and (random() < 0.1 or not ObjectInWater(oObj, 1)) and
      not IsCollideY(oObj, 1) then return AdjustPosY(oObj, 1) end;
    -- If we can go up one pixel extra then do so
    if iAdjY < 0 and random() < 0.1 and ObjectInWater(oObj, -1) and
      not IsCollideY(oObj, -1) then return AdjustPosY(oObj, -1) end;
  end
